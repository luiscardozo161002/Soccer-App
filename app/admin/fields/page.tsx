...
function EditFieldModal({ field, onClose }: { field: FieldType | null; onClose: () => void }) {
  ...
  useEffect(() => {
    if (field) reset({ name: field.name, location: field.location ?? "" });
  }, [field, reset]);
  ...
  return (
    <Modal open={!!field} onClose={onClose} title="Editar cancha">
...
export default function FieldsPage() {
  ...
  const { data, isLoading, isError } = useFields(page, FIELDS_PAGE_SIZE);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-slate-500">{data?.meta.totalItems ?? 0} cancha(s) registrada(s).</p>
      </div>
